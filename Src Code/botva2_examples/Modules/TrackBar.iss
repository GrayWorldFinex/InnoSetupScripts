[Code]
//модуль для работы с TrackBar'ами. для работы требует подключения botva2.iss
...
//Parent    - хэндл родительского окна
//BkgLeft,
//BkgTop,
//BkgWidth,
//BkgHeight - координаты левого верхнего угла и ширина/высота фоновой картинки в координатах родительского окна (Parent)
//MinX,
//MaxX      - крайнее левое/правое положение по оси Х, в пределах которых может двигаться кнопка (бегунок) трэкбара в координатах Parent
//BtnTop,
//BtnWidth,
//BtnHeight,
//BtnShadowWidth - см. аналогичные параметры в BtnCreate

//во всех последующих функциях/процедурах ind - значение, которое вернула функция ImgTBCreate

//удалить трэкбар
//procedure ImgTkDelete(ind:integer);

//получить текущую позицию трэкбара (0-100)
//function ImgTBGetPosition(ind:integer):integer;

//установить текущую позицию трэкбара (0-100)
//procedure ImgTBSetPosition(ind, pos:integer);

//установить видимость трэкбара
//procedure ImgTBSetVisibility(ind:integer; Value:boolean);

//установить процедуру, которая будет выполняться при смене позиции трэкбара
...
begin
  ...
  case Msg of
    ...
    $200: if ATB[trind].IsMouseDown then begin
      ...
      ImgSetVisibility(ATB[trind].bkgimg,not ImgGetVisibility(ATB[trind].bkgimg)); //хрень вобщем-то написана и лишняя нагрузка на процессор
      ImgSetVisibility(ATB[trind].bkgimg,not ImgGetVisibility(ATB[trind].bkgimg)); //но без этих строк нормально
      ImgApplyChanges(Parent);                                                     //не перерисовывается кнопка (бегунок трэкбара)
    end;
  end;
end;
...
//  Last,i:integer;
begin
//нельзя удалять элемент массива
